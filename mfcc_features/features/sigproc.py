...
def framesig(sig,frame_len,frame_step,winfunc='hamm',frid=0,fs=16000):
    ...
    if winfunc == 'box':
        windoapp = lambda x:numpy.ones((1,x))
        win = numpy.tile(windoapp(frame_len),(numframes,1))
    elif winfunc == 'hamm':
        win = numpy.tile(numpy.hamming(frame_len),(numframes,1))
    else:
        print 'Give a Window Type for winfunc..hamm or box'
        sys.exit()
    #print 'Framing Data'
    ...
    if frid:
        st = indices[:,0]
        ed = indices[:,-1]+1
        tms = numpy.hstack((st.reshape(st.shape[0],1),ed.reshape(ed.shape[0],1)))
        return frames*win, tms/float(fs)
    else:
        return frames*win
...
def magspec(frames,NFFT):
    ...
    ########### Duplicate freq are removed automatically in numpy fft################
...
def powspec(frames,NFFT):
    ...
    """    
    #not sure if normalization is usually done or not..removing it for now based on Dan Ellis implementation and matlab HTK implementation
